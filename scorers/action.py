...
def evaluate_action_classification(gt_file='ROSCO_GT.csv', pred_file='qwen3int.csv'):
    """
    Evaluate action classification by comparing predictions with ground truth.
    ...
    merged_df = pd.merge(
        gt_df[['sample_name', 'ACTION1_ID', 'ACTION2_ID', 'ACTION3_ID']], 
    ...
    # Filter out unlabeled samples (all ACTION IDs are -1)
    unlabeled_mask = (
    ...
    print(f"Unlabeled GT samples (all labels = -1): {num_unlabeled}")
    ...
    if len(merged_df) == 0:
        print("ERROR: No valid samples remaining after filtering!")
        return
    ...
    # Using ACTION1_ID vs Final_Class for a strict single-label proxy
    valid_mcc_rows = merged_df[merged_df['ACTION1_ID'] != -1]
    mcc_score = matthews_corrcoef(valid_mcc_rows['ACTION1_ID'], valid_mcc_rows['Final_Class'])
    print(f"\n[Analysis 2] Matthews Correlation Coefficient (GT=ACTION1): {mcc_score:.4f}")
    ...
    print("\n[Analysis 4] Top 5 Most Frequent Confusions (GT=ACTION1 vs Pred):")
    # Filter for errors only
    errors = valid_mcc_rows[valid_mcc_rows['ACTION1_ID'] != valid_mcc_rows['Final_Class']]
    if len(errors) > 0:
        error_counts = errors.groupby(['ACTION1_ID', 'Final_Class']).size().reset_index(name='Count')
    ...
    print(f"MCC Score (Strict ACTION1 match): {mcc_score:.4f}")
...
if __name__ == "__main__":
    results = evaluate_action_classification(
        ...
        'ACL_LLM/llm_output/g3flash_icl_action.csv'
    )
